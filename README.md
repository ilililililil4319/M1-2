...
> **Firestore가 뭔가요?** 쉽게 말하면 **인터넷에 있는 엑셀 시트 같은 저장 공간**이다. `naver_financials.csv`처럼 내 컴퓨터 안에만 있는 파일은 웹사이트(Vercel)나 서버(Render)가 직접 열어볼 수 없다. 그래서 정제된 212개 데이터를 CSV에서 꺼내 Firestore라는 온라인 저장소에 옮겨두면, 어디서 접속하든(집 PC, 배포된 서버, 사용자의 브라우저) 항상 같은 최신 데이터를 실시간으로 읽고 쓸 수 있게 된다. 이 프로젝트에서는 Google의 Firebase 서비스가 제공하는 Firestore를 데이터베이스로 사용했다.
